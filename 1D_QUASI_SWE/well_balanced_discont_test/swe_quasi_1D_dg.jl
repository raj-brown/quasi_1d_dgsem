...
function initial_condition(x)
    ...
    if (xl <= x <= xr)
        #A = (1 - sigma_0*(1 + cos(2.0*pi*(x - (xl + xr)/2.0)/(xr - xl))))
        A = (1 - sigma_0*(1 + cos(2.0*pi*(x - (xl-xr)/2.0)/(xr-xl))))
...
# cell_centers_x = repeat(sum(md.x, dims=1) / size(md.x, 1), size(md.x, 1), 1)
# tol = 100 * eps()
# x_modified = @. (1 - tol) * (md.x - cell_centers_x) + cell_centers_x # squeeze the nodal values towards cell centers
xq = Vq * md.x
#md.x .= x_modified
...
sol = solve(ode, RK4(), adaptive=false, dt = 0.01 * h, callback=AliveCallback(alive_interval=10), 
            saveat=LinRange(tspan..., NT_save), abstol=1e-12, reltol=1e-12)
