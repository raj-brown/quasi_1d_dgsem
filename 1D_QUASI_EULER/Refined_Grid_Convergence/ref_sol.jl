...
function initial_condition(x, equations::CompressibleEulerEquations1D) 
    ...
    #rho = 1.0 + 0.2 * (1 + sin(pi * (x - 0.5) / 0.5))
    u   = 0.0
...
println("Computing...")
sol = solve(ode, RK4(), saveat=LinRange(tspan..., 50), abstol=1e-10, reltol=1e-10, callback=AliveCallback(alive_interval=10))
sol_md = "Rev_Ref_sol_N_"*string(N)*"_K_"*string(K)*".mat"
u = sol.u[end]
rhoA = getindex.(u, 1)
rhouA = getindex.(u, 2)
EA = getindex.(u, 3)
